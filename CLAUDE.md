...
- [~] Expand the proto over Connect RPC (flows to Rust + TS clients via `proto:gen`): **done** — Geocode, ReverseGeocode, Directions, Elevation, TimeZone, TextSearch, **DistanceMatrix, PlacesAutocomplete, PlacesNearby**; `Place` now carries lat/lng + place_id and `Route` carries numeric distance_meters/duration_seconds; **remaining** — roads (snap/nearest), address validation, places-new (place details / photos). Note: DistanceMatrix takes coordinate `LatLng` lists (Waypoint only converts from LatLng), so clients geocode addresses first. autocomplete pulls `uuid` → crate enables uuid's wasm `js` backend only on the `worker` feature.
